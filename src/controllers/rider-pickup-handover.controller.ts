...
export class RiderPickupHandoverController {
  ...
  private async generateHandoverCode(): Promise<string> {
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      const existing = await this.pickupHandoverRepository.findOne({
        where: {handoverCode: code, status: PickupHandoverStatus.PENDING} as object,
      });
  ...
  @response(200, {description: "The rider's own picked-up requests not yet in a pending handover batch"})
  async handoverEligible(
    ...
    const pickups = await this.pickupRequestRepository.find({
      where: {assignedRiderId: rider.id, isDeleted: false, status: PickupRequestStatus.PICKED_UP} as object,
  ...
  @response(200, {description: 'Pickup handover batch submitted, with a code to show the receiver'})
  async submitHandover(
        ...
        'application/json': {
          schema: {
            ...
            properties: {
              ...
              handoverToType: {type: 'string', enum: Object.values(PickupHandoverTargetType)},
              handoverToStoreId: {type: 'string', format: 'uuid', description: 'Required when handoverToType is "store".'},
              handoverToRiderId: {
                type: 'string',
                format: 'uuid',
                description: 'Required when handoverToType is "rider" — covers both "Van" and "Rider" in the app UI.',
              },
              remarks: {type: 'string'},
            },
          },
    ...
    const invalid = pickups.filter(
      p => p.assignedRiderId !== rider.id || p.status !== PickupRequestStatus.PICKED_UP,
    );
    if (invalid.length) {
      throw new HttpErrors.BadRequest('One or more pickup requests are not yours to hand over, or not picked up yet.');
    }
    ...
    if (alreadyBatchedItems.length) {
      const activeHandovers = await this.pickupHandoverRepository.find({
        where: {
          id: {inq: [...new Set(alreadyBatchedItems.map(i => i.pickupHandoverId))]},
          status: PickupHandoverStatus.PENDING,
        } as object,
        fields: {id: true} as object,
      });
      const activeHandoverIds = new Set(activeHandovers.map(h => h.id));
      if (alreadyBatchedItems.some(i => activeHandoverIds.has(i.pickupHandoverId))) {
        throw new HttpErrors.BadRequest('One or more pickup requests are already in a pending handover batch.');
      }
    }
  ...
  async myHandovers(
    ...
    const items = handoverIds.length
      ? await this.pickupHandoverItemRepository.find({where: {pickupHandoverId: {inq: handoverIds}} as object})
      : [];
  ...
  @response(200, {description: 'Pickup handover batches directed to the calling rider by another rider'})
  async incomingHandovers(
    ...
    const handovers = await this.pickupHandoverRepository.find({
      where: {
        ...
        isDeleted: false,
        status: tab === 'completed' ? PickupHandoverStatus.CONFIRMED : PickupHandoverStatus.PENDING,
      } as object,
      order: ['submittedAt DESC'],
    });
    const handoverIds = handovers.map(h => h.id);
  ...
  async confirmIncomingHandover(
    ...
    if (handover.handoverToType !== PickupHandoverTargetType.RIDER || handover.handoverToRiderId !== rider.id) {
