...
export class StorePriceOverrideController {
  ...
  @response(200, {
    ...
    content: {
      'application/json': {schema: getModelSchemaRef(StorePriceOverride)},
    },
  })
  async create(
    @requestBody({
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: {
          type: 'array',
          items: getModelSchemaRef(StorePriceOverride, {
  ...
  async find(
    @param.filter(StorePriceOverride) filter?: Filter<StorePriceOverride>,
  ): Promise<StorePriceOverride[]> {
    return this.storePriceOverrideRepository.find(filter);
  }
  ...
  async updateAll(
    ...
    return this.storePriceOverrideRepository.updateAll(
      storePriceOverride,
      where,
    );
  }
  ...
  async findById(
    ...
    @param.filter(StorePriceOverride, {exclude: 'where'})
    filter?: FilterExcludingWhere<StorePriceOverride>,
  ): Promise<StorePriceOverride> {
    return this.storePriceOverrideRepository.findById(id, filter);
  }
  ...
  @authenticate('jwt')
  @authorize({roles: ['super_admin']})
  @put('/store-price-overrides/{id}')
  @response(204, {description: 'StorePriceOverride PUT success'})
  async replaceById(
    @param.path.string('id') id: string,
    @requestBody() storePriceOverride: StorePriceOverride,
  ): Promise<void> {
    await this.storePriceOverrideRepository.replaceById(id, storePriceOverride);
  }
