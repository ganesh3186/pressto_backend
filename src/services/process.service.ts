...
export class ProcessService {
  ...
  constructor(
    ...
    private spmRepo: ServiceProcessMappingRepository,
  ...
  async initProcess(garmentId: string, _initiatedBy: string): Promise<object> {
    ...
    if (existing.count > 0) {
      throw new HttpErrors.Conflict(
        'Process already initialised for this garment.',
      );
    }
  ...
  async advanceProcess(
    ...
    if (this.qrScanRequired) {
      if (!qrCode) {
        throw new HttpErrors.BadRequest(
          'QR code is required (QR_SCAN_REQUIRED is enabled).',
        );
      }
    ...
    try {
      ...
      const inProgressLog = allLogs.find(
        l => l.status === ProcessLogStatus.IN_PROGRESS,
      );
      const nextPendingLog = allLogs.find(
        l => l.status === ProcessLogStatus.PENDING,
      );
      ...
      if (!inProgressLog) {
        ...
        const allDone = allLogs.every(
          l =>
            l.status === ProcessLogStatus.COMPLETED ||
            l.status === ProcessLogStatus.SKIPPED,
        );
        if (allDone) {
          throw new HttpErrors.Conflict(
            'All process steps are already completed.',
          );
        }
        throw new HttpErrors.BadRequest(
          'No step is currently in progress. Re-check process state.',
        );
      }
  ...
  async completeAllProcesses(
    ...
    try {
      ...
      if (!allLogs.length) {
        ...
        if (!rows.length) {
          if (this.processingEnabled) {
            ...
            await this.garmentStatusHistoryRepo.create(
              {
                ...
                changedBy: performedBy,
                remarks:
                  'Processing disabled — no process steps configured, marked done directly',
              },
              {transaction: tx},
            );
            ...
            return {
              message:
                'All process steps completed. Garment moved to Quality Check.',
              stepsCompleted: 0,
      ...
      const remaining = allLogs.filter(
        l =>
          l.status === ProcessLogStatus.PENDING ||
          l.status === ProcessLogStatus.IN_PROGRESS,
      );
      if (!remaining.length) {
        throw new HttpErrors.Conflict(
          'All process steps are already completed.',
  ...
  async fastTrackToReady(
    ...
    if (this.processingEnabled) {
      throw new HttpErrors.BadRequest(
        'Processing is enabled — follow the standard stage-by-stage flow.',
      );
    }
    ...
    if (
      garment.status !== GarmentStatus.IN_PROCESS &&
      garment.status !== GarmentStatus.QUALITY_CHECK
    ) {
      throw new HttpErrors.BadRequest(
    ...
    if (garment.status === GarmentStatus.IN_PROCESS) {
      const result = (await this.completeAllProcesses(
        garmentId,
        performedBy,
        qrCode,
      )) as {stepsCompleted: number};
    ...
    await this.garmentStatusHistoryRepo.create({
      ...
      changedBy: performedBy,
      remarks:
        'Fast-tracked to ready — processing disabled (PROCESSING_ENABLED=false)',
    });
  ...
  async reverseStep(garmentId: string, performedBy: string): Promise<object> {
    ...
    if (!allLogs.length) {
      throw new HttpErrors.BadRequest(
        'Process not initialised for this garment.',
      );
    }

    // Find the last completed step
    const completedLogs = allLogs.filter(
      l => l.status === ProcessLogStatus.COMPLETED,
    );
  ...
  async syncOrderStatusFromGarment(
    garmentId: string,
    changedBy: string,
  ): Promise<void> {
    const {v4} = await import('uuid');
    ...
    if (!garment) return;
    const anchorItem = await this.orderItemRepo.findOne({
      where: {id: garment.orderItemId},
    });
    if (!anchorItem) return;
    const order = await this.orderRepo.findOne({
      where: {id: anchorItem.orderId, isDeleted: false},
    });
    if (!order) return;
    ...
    if (activeStatuses.length === 0) {
      await setOrderStatus(
        ...
        'Auto-cancelled: all garments returned/on hold',
      );
      return;
    }
    ...
    if (derived) {
      await setOrderStatus(
        derived,
        'Auto-synced from garment processing pipeline',
      );
    }
  }
  ...
  private async resolveGarmentTracking(garment: {
    ...
    const storeIds = [
      ...new Set(
        [homeStoreId, transfer?.fromStoreId, transfer?.toStoreId].filter(
          Boolean,
        ),
      ),
    ] as string[];
    const stores = storeIds.length
      ? await this.storeRepo.find({where: {id: {inq: storeIds}}})
      : [];
    ...
    return {
      homeStoreId,
      homeStoreName: homeStoreId
        ? (storeNameById.get(homeStoreId) ?? null)
        : null,
      currentStoreId,
      currentStoreName: currentStoreId
        ? (storeNameById.get(currentStoreId) ?? null)
        : null,
  ...
  async getProcessStatus(garmentId: string): Promise<object> {
    ...
    const serviceList = [];
    for (const [seq, groupLogs] of [...serviceGroups.entries()].sort(
      (a, b) => a[0] - b[0],
    )) {
      const serviceId = groupLogs[0].serviceId;
      const svc = serviceMap.get(serviceId);

      const allCompleted = groupLogs.every(
        l =>
          l.status === ProcessLogStatus.COMPLETED ||
          l.status === ProcessLogStatus.SKIPPED,
      );
      const anyInProgress = groupLogs.some(
        l => l.status === ProcessLogStatus.IN_PROGRESS,
      );
    ...
    const currentStep =
      logs.find(l => l.status === ProcessLogStatus.IN_PROGRESS) ?? null;
